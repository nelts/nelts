...
const plugin_2 = require("../agent/plugin");
...
function MakeAgentPluginRender(agentName, app) {
    const base = app.base;
    let node_module_paths = find_node_modules_1.default({ cwd: base, relative: false });
    if (!node_module_paths.length)
        throw new Error('cannot find node_modules path');
    const node_module_path = node_module_paths[0];
    async function dispatch(component_path, root) {
        const { name, dependenties } = plugin_collect_dependencies_1.default(component_path, node_module_path, { env: app.env, isWorker: false, agentName });
        if (!app.plugins[name])
            app.plugins[name] = new plugin_2.default(app, name, component_path);
        if (!root)
            root = app.plugins[name];
        app.plugins[name].root = root;
        const childrens = await Promise.all(dependenties.map(dep => dispatch(dep, root)));
...
exports.MakeAgentPluginRender = MakeAgentPluginRender;
